...
var lightClientIp = '10.30.64.173';
...
oscServer.on('/BCI', function(msg) {
    receiveBCI(msg);
});


function receiveBCI(msg){
    console.log('...BCI signal!')
    console.log(msg)  
    behaviourScene(msg[1])
}
//////////////////////////////n////////////////////////
...
function processingSleep(){
    ...
    amatriadata.muteState = true; // With periodic updates we need to update the mute state, or it will get unmuted after 10 seconds
    amatriadata.manualMute = false; // track manual vs sleep/wake mutes with a property
    amatriadata.sleepState = true;
    console.log(Date(Date.now())+ 'sleeping Amatria')
    rewriteData(amatriadata);
}

function processingWake(){
    behaviourOscClient.send('/serverMessage/wake')
    if (!amatriadata.manualMute) {   // this means NOT manually muted... 
        audioOscClient.send('/masterMute', 1);  // wasn't manually muted, so unmute when we wake up
    }
    amatriadata.sleepState = false;
    console.log(Date(Date.now())+ 'Waking Amatria')
    rewriteData(amatriadata);
}
...
function sendMuteState(){
    ...
    var muteValue = 0;
    var volume = amatriadata.masterVolume
    if (!amatriadata.muteState){muteValue=1} // if mutestate is false, send unmute
    audioOscClient.send('/masterMute', muteValue);  //send the current mute state
    audioOscClient.send('/masterVolume', parseFloat(volume));
    //console.log(Date(Date.now()) + 'updateing max mute state: ' + amatriadata.muteState)
...
function createTimerCallback(values){
    ...
    sleepTimer = setTimeout(function(){
        console.log(Date(Date.now())+ 'Sleep Timer Triggered, Timer enabled?: ' + amatriadata.timerActive)
        if (amatriadata.timerActive){
            processingSleep()
        }},timeTillSleep)

    wakeTimer = setTimeout(function(){
        console.log(Date(Date.now())+ 'Wake Timer Triggered, Timer enabled?: ' + amatriadata.timerActive)
        if (amatriadata.timerActive){
            processingWake()
        }},timeTillWake)   
}
...
if (!fs.existsSync('amatriadata.json')){
    var emptyJson = {};
    fs.appendFile('amatriadata.json',JSON.stringify(emptyJson,null,2),callback)
}
else{
    callback();
}
...
// Startup Callback
function callback(){
    var data = fs.readFileSync('amatriadata.json');
    amatriadata = JSON.parse(data);
    console.log(amatriadata);
    checkTimers();
...
function setVolumeGeneric(values){
    ...
    if (ids[0] == -1){
        ...
        for (var i = ids[0]; i<=ids[1]; i++){
            ...
            if (ids[0] == 1){amatriadata.externalVolume = volume;}
            if (ids[0] == 25){amatriadata.internalVolume = volume;}
        }
...
function setBehaviourIntensity(values){
   ...
   // console.log( Date(Date.now()) + 'BehaviourIntensity: ' + values);
    amatriadata.behaviourIntensity = values;
    rewriteData(amatriadata);
...
function rewriteData(amatriadata){
    fs.writeFile('amatriadata.json',JSON.stringify(amatriadata,null,2),emitUpdate);
}
...
function emitUpdate(result){
    ...
    //    console.log( Date(Date.now()) + 'broadcasting')
        socket.broadcast.emit('updateProps',amatriadata);
        socket.emit('updateProps',amatriadata);
    }
}
